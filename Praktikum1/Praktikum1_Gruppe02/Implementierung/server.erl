...
dispatcher_loop(HBQ, DLQ, ClientList, ConfigListe, Latency, ClientLifetime, ServerName, DlqLimit, FreeMSGID, LogFileName) ->
	...
	receive
		%Empfange eine Anfrage nach einer neuen MSGID
		{getmsgid, PID} ->
			PID ! {nid, FreeMSGID},
			...
			;
		%Empfange eine neue Nachricht von einem Redakteur
		{dropmessage, {Nachricht, Number}} ->
			{NewHBQ, NewDLQ} = hbq:dropmessage(HBQ, DLQ, {Nachricht, Number}),
			{Msg, COut, _, _, _} = Nachricht,
			util:logging(LogFileName, Msg ++ " "++ integer_to_list(Number) ++ "te_Nachricht. COut: " ++ util:timestamp_to_list(COut) ++ " HBQ In: " ++ util:timeMilliSecond() ++ " -dropmessage\n"),
			dispatcher_loop(NewHBQ, NewDLQ, ClientList, ConfigListe, Latency, ClientLifetime, ServerName, DlqLimit, FreeMSGID, LogFileName)
			;
		%Empfange eine Nachricht von einem Leser der eine neue Nachricht lesen moechte
		{getmessages, PID} ->
		...
		%Wenn sich zu lange kein Client gemeldet hat, dann fahre den Server runter
	after
		Latency * 1000 ->
